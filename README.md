<!DOCTYPE html>

<html lang="en">
<head>
    <h1 align="center">Nga'ay ho kamo! Ini ko Mirusa. </h1>
</head>
  
<body>
  <div align="center" width="25%">
    <img src="https://media.tenor.com/1imhldTQXEoAAAAd/mushoku-tensei-anime.gif">
  </div>
  
<div style="height= auto; width= auto;">
  <br>
    <h2 align="center">About me</h2>
    <div>
      <img src="https://media.tenor.com/7xJ6UHojnk8AAAAd/roxy-migurdia.gif" width="30%" align="left">
      <li><b> Name:</b> Mirusa Usiliq</li>
      <li><b> Age:</b> 18</li>
      <li><b> Nationality:</b> Taiwanese</li>
      <li><b> Hobbies:</b> Anime, Manga, Languages, Linguistics, Sciences</li>
      <li><b> Pronouns:</b> She/Her ʕ •ᴥ•ʔ</li>
      <li><b> Interests:</b> Computer Science, Bioinformatics, Statistics and Math</li>  
      <li><b> Languages:</b> Mandarin (Native), Hokkien (Native), Hakka (Native), English (Fluent), Amis (Fluent), Atayal (Fluent), Portuguese (Beginner), Japanese (Beginner)</li>
    </div>
  </br>
</div>
    
<div></div>

<div>
  <br>
    <h2 align="center">Knowledge</h2>
    <div>
        <img src="https://media.tenor.com/opNarol2l_4AAAAd/roxy.gif" width="30%" align="right">
        <li><b>Programming Languages: </b>C, C#, C++, Python</li>
        <li><b>Learning now: </b> HTML, CSS, JavaScript, Assembly, R, Swift</li>
        <li><b>Education: </b> Computer Science (Ongoing)</li>
        <p></p>
        <p>I'm currently studying in NDHU and working on a lot of projects... so yeah... ʕ•ᴥ• ʔ</p>
    </div>
  </br>
</div>

<div></div> 
  
<div>
  <br>
    <h2 align="center">Contact me</h2>
    <img src="https://media.tenor.com/pMkPqfRimeYAAAAd/shock-roxy-shock.gif" width="20%" align="left">
    <p><b>These are the social media I use frequently recently...</b></p>
    <div>
      <li><b>Twitter: </b> <a href="https://twitter.com/mirusausiliq"><b>@mirusausiliq</b></a></li>
      <li><b>Discord: mirusausiliq#6635</b></li>
    </div>
    <p></p>
    <p><b>Actually I'm more active on Discord, so if you have any ideas or sth., please don't be hesitate and contact me. :D</b></p>
<div align="center"><h2>Thank you for visiting my profile!</h2>
    <img src="https://media.tenor.com/RGm8YswOKdcAAAAd/mushoku-tensei-anime.gif" width=75%></div>
</div>

</body>
</html>


<!---
mirusausiliq/mirusausiliq is a ✨ special ✨ repository because its `README.md` (this file) appears on your GitHub profile.
You can click the Preview link to take a look at your changes.
--->
